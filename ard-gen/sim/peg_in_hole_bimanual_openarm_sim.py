...
했는데(최대 7cm, 발산), CMA-ES가 찾은 값(현재 파일에 반영됨: 베이스
kp=2081.80/kv=1565.01, 중간 kp=145.00/kv=1052.43 -- 놀랍게도 kp보다 kv가
훨씬 큼, 손목 kp=358.48/kv=569.50, _NULLSPACE_GAIN=0.052)에서는 드리프트가
**한계 진동(bounded oscillation)으로 바뀌었다** -- dx는 대략 -30~+10mm,
dy는 대략 0~26mm 사이를 오가며 수렴하지도 발산하지도 않고 계속 진동한다
(1000스텝까지 실측 확인, 더 돌려도 발산은 안 하는 것으로 보임). 그런데
dy가 0 근처로 안 돌아오는 계통적 편향(systematic bias)이 있어서, dx가
0에 가까워지는 순간에도 dy가 hole 반경(outer_half=0.0195m)보다 커서
"xy_within_hole_footprint" 판정을 통과하는 순간이 안 나온다 -- 그래서
depth는 1000스텝 내내 0으로 남는다.

발견한 것 중 특히 의외였던 점: CMA-ES가 수렴한 kv 값들이 거의 다 그
관절의 kp보다 크거나 비슷하다(특히 중간 관절은 kv가 kp의 7배!) --
...
다음에 이어서 할 사람에게: 이 시점에서 (a) dy의 계통적 편향(왜 0이
아니라 +25mm 근처에서 진동하는지)을 더 찾아보거나, (b) 비용 함수에
"dx와 dy가 동시에 hole 반경 이내인 스텝이 있었는지"를 직접 넣어서
CMA-ES를 더 오래(지금은 popsize=6, 10세대 두 번뿐이었다) 돌리거나,
(c) 아예 완전한 역동역학 제어로 바꾸는 게 다음 단계로 보인다.
optimize/ 디렉터리에 이 탐색 스크립트를 정식으로 옮겨서 재현 가능하게
만드는 것도 다음 작업이다(지금은 /tmp에서 임시로 돌린 것이라 저장 안 됨
-- 이 문단의 수치와 파라미터 값이 유일한 기록).
"""
...
_IK_MAX_ITERS = 200
_IK_STEP_SCALE = 0.5
_NULLSPACE_GAIN = 0.052  # 근거: _advance_virtual docstring 참고 (CMA-ES 게인 탐색으로 재조정)
